    ...
    def create_xls(self, fields, rows):
        ...
        for row in rows:
            ...
            for p in payslips:
                ...
                if amount:
                    paid=amount[0]
                ws.write(len(payslip),0,str(p.employee_id.bank_account_id.acc_number or ''))
                ws.write(len(payslip),1,str(paid))
                ws.write(len(payslip),2,str(p.employee_id.fingerprint_code))
                ws.write(len(payslip),3,str(p.employee_id.name))
                ws.write(len(payslip),4,str(p.employee_id.department_id.name))
